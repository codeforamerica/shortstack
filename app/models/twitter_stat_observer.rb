class TwitterStatObserver < ActiveRecord::Observer
  def after_save(stat)
    ...
    summary.update_attributes(:followers_count => stat.followers_count, :following_count => stat.following_count, :listed_count => stat.listed_count, :statuses_count => stat.statuses_count )
      ...
      TwitterSummary.create(:organization => stat.link.linkable, :link => stat.link, :followers_count => stat.followers_count  , :following_count => stat.following_count, :listed_count => stat.listed_count, :statuses_count => stat.statuses_count )
    end  
  end
end